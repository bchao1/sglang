...
# --- 480p series ---
ax.plot(deltas_curve, speeds_curve_480,
        color=C480, linewidth=2.5, alpha=0.9, zorder=2, label="480×832")
ax.fill_between(deltas_curve, 1.0, speeds_curve_480,
                alpha=0.12, color=C480, zorder=1)
...
# --- 720p series ---
ax.plot(deltas_curve_720, speeds_curve_720,
        color=C720, linewidth=2.5, alpha=0.9, zorder=2, label="720×1280",
        linestyle="--")
ax.fill_between(deltas_curve_720, 1.0, speeds_curve_720,
                alpha=0.10, color=C720, zorder=1)
...
ann_480 = {
    0.01: dict(xytext=(0.013, 1.42), color="#ccc",
               label=f"δ=0.01  {MEASURED_480[0.01]:.2f}×"),
    0.02: dict(xytext=(0.026, 1.62), color="#ccc",
               label=f"δ=0.02  {MEASURED_480[0.02]:.2f}×"),
    0.05: dict(xytext=(0.063, 2.10), color=C_REC,
               label=f"δ=0.05  {MEASURED_480[0.05]:.2f}×  ★"),
    0.10: dict(xytext=(0.073, 2.58), color="#ccc",
               label=f"δ=0.10  {MEASURED_480[0.10]:.2f}×"),
}
for d, cfg in ann_480.items():
    ax.annotate(
        f"  {cfg['label']}",
        xy=(d, MEASURED_480[d]), xytext=cfg["xytext"],
...
ann_720 = {
    0.01: dict(xytext=(0.0055, 2.38), color=C720,
               label=f"δ=0.01  {MEASURED_720[0.01]:.2f}×"),
    0.05: dict(xytext=(0.062, 3.14), color=C_REC,
               label=f"δ=0.05  {MEASURED_720[0.05]:.2f}×  ★"),
}
for d, cfg in ann_720.items():
    ax.annotate(
        f"  {cfg['label']}",
        xy=(d, MEASURED_720[d]), xytext=cfg["xytext"],
        fontsize=9.5, color=cfg["color"],
        fontweight="bold" if d == RECOMMENDED else "normal",
        arrowprops=dict(arrowstyle="->", color=cfg["color"], lw=1.2),
    )
...
ax.text(0.074, 3.48, "← higher δ = more low-res steps", color="#888", fontsize=9, style="italic")

# --- Legend ---
legend = ax.legend(
    title="Resolution", title_fontsize=10,
