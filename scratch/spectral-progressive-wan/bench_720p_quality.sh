#!/usr/bin/env bash
# =============================================================================
# Wan T2V 720p Progressive Quality Sweep — δ=0.01 and δ=0.05 only
# 10 cinematic prompts × 2 delta values = 20 videos across 3 GPUs.
#
# Note: Wan's patch embedding requires even spatial latent dims.
# Our WanProgressiveDenoisingStage.forward() auto-aligns 720×1280 → 704×1280
# (H_lat 90→88, init_h_lat=44 even) so videos render at 704p.
# =============================================================================
set -euo pipefail
...
RESULTS_DIR="$SCRIPT_DIR/results/quality_720p_${TS}"
mkdir -p "$RESULTS_DIR"
...
run_gpu_worker() {
    ...
    for pidx in "${prompt_indices[@]}"; do
        ...
        echo "[GPU $gpu_id] $pid_label: ${prompt:0:60}..."

        for delta in "${DELTAS[@]}"; do
            local dlabel=$(echo "$delta" | tr '.' '_')
            local label="${pid_label}_d${dlabel}"
            ...
            flock "$TIMING_LOG" \
                echo -e "${pid_label}\td${dlabel}\t${total_s}\t${denoise_s}\t${trans_step}\tGPU${gpu_id}" \
                >> "$TIMING_LOG"
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║  720p QUALITY SWEEP COMPLETE                 ║"
echo "╚══════════════════════════════════════════════╝"
echo "Results: $RESULTS_DIR"
column -t -s $'\t' "$TIMING_LOG"
echo "Videos: $(ls "$RESULTS_DIR"/*.mp4 2>/dev/null | wc -l)/20"
$ALL_OK || { echo "Some runs failed — check *.log files"; exit 1; }
