#!/usr/bin/env python3
"""
Generate fullres vs progressive side-by-side comparisons for Z-Image PR.

Runs sglang generate per call, extracts DENOISING-ONLY timing from logs,
saves comparison strips + timing.json for downstream PR analysis scripts.

Usage:
  CUDA_VISIBLE_DEVICES=0 python3 scratch/spectral-progressive-z-image/gen_pr_comparisons_zimage.py
"""
...
OUT_DIR = SCRATCH / "results" / "pr_images"
DOC_DIR = SCRATCH.parent.parent / "docs_new" / "images" / "progressive_zimage"
...
def parse_denoise_time(log: str, is_progressive: bool) -> float | None:
    if is_progressive:
    ...
    # Fallback: "Pixel data generated successfully in X.XXX seconds"
    m = re.search(r"generated successfully in ([0-9.]+)", log)
    if m:
        return float(m.group(1))
    return None
...
def make_side_by_side(img_fr, img_pr, label, den_fr, den_pr, out_path, thumb_w=512):
    ...
    lbl_fr = make_label_image(f"Fullres  denoise={den_fr:.1f}s", thumb_w)
    lbl_pr = make_label_image(
        f"Progressive δ=0.05  denoise={den_pr:.1f}s  ({speedup:.2f}×)",
        thumb_w,
        bg=(0, 60, 30),
    )
    ...
    print(f"  Saved strip: {out_path.name}")


def make_montage(strip_paths, out_path, cols=2):
...
def run_generate(prompt, outfile, extra_flags=None):
    cmd = (
        ["sglang", "generate"]
        + BASE_FLAGS
    ...
    is_prog = bool(extra_flags)
    denoise = parse_denoise_time(log, is_prog)
...
def run():
    ...
    print(f"Timing: DENOISING LOOP ONLY")
    print()
    ...
    for i, (label, prompt) in enumerate(PROMPTS, 1):
        ...
        if not ok_fr:
            ...
            continue
        print(
            f"  fullres:     wall={wall_fr:.1f}s  denoise={f'{den_fr:.2f}s' if den_fr else 'N/A'}"
        )
        ...
        if den_fr and den_pr:
            ...
            print(f"  progressive: wall={wall_pr:.1f}s  (wall speedup={speedup:.2f}×)")
        ...
        d_fr_plot = den_fr or wall_fr
        d_pr_plot = den_pr or wall_pr
